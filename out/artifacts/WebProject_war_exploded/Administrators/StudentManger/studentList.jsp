...
<head>
    ...
    <script>
        function findcourse(username) {
            // if (confirm("确定删除？")) {
                location.href = "${pageContext.request.contextPath}/findOneForm?username=" + username;
            // }
        }
...
<div class="container">
    ...
    <div style="float: right;margin: 25px;">
        <a class="btn " href="${pageContext.request.contextPath}/add.jsp"></a>
        ...
        <table border="1" class="table table-bordered table-hover">
            ...
            <c:forEach items="${pb.list}" var="user" varStatus="s">
                <tr>
                    ...
                    <td>${user.major}</td>
                    <td>
                        <a class="btn btn-default btn-sm" href="javascript:findcourse(${user.username});">课表查询</a></td><%--删除操作--%>
                </tr>
                <%--<a class="btn btn-default btn-sm"--%>
                   <%--href="${pageContext.request.contextPath}/findUserServlet?id=${user.username}">修改</a>&nbsp;--%>
            </c:forEach>
        </table>
    </form>
